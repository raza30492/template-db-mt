...
class Login extends Component {
  ...
  constructor() {
    ...
    self._renderForgotPasswdLayer = this._renderForgotPasswdLayer.bind(this);
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.user.authProgress &&
      !nextProps.user.authProgress &&
    ...
    if (!this.props.user.authenticated && nextProps.user.authenticated) {
      this.props.dispatch(searchUser(window.sessionStorage.email));
    }
  ...
  _forgotPassword(operation: string, event: Event): void {
    const { credential } = this.state;
    let error = {};
    ...
    if (operation == "SEND_OTP") {
      ...
      axios
        .put(
          window.serviceHost +
            "/v2/misc/forgot_password/send_otp?email=" +
            credential.email
        )
        .then(response => {
      ...
      axios
        .put(
          window.serviceHost +
            "/v2/misc/forgot_password/verify_otp?email=" +
            credential.email +
            "&otp=" +
            credential.otp
        )
      ...
      if (credential.email == undefined || credential.email == "") {
        ...
        credential.newPassword == undefined ||
      ...
      axios
        .put(
          window.serviceHost +
            "/v2/misc/forgot_password/change_password?email=" +
            credential.email +
            "&otp=" +
            credential.otp +
            "&newPassword=" +
  ...
  _renderForgotPasswdLayer(): ?Object {
    const {
      credential,
      changing,
      error,
    ...
    if (changing) {
      ...
      if (!otpSent && !otpVerified) {
        forgotBtnControl = (
          <Button
            ...
            label="Submit"
            primary={true}
            onClick={this._forgotPassword.bind(this, "SEND_OTP")}
          />
        );
        ...
        forgotBtnControl = (
          <Button
            ...
            label="Verify OTP"
            primary={true}
            onClick={this._forgotPassword.bind(this, "VERIFY_OTP")}
          />
        );
      } else if (otpSent && otpVerified) {
        forgotBtnControl = (
          <Button
            ...
            primary={true}
            onClick={this._forgotPassword.bind(this, "CHANGE_PASSWORD")}
          />
        );
      }
      let otpInput;
      ...
      return (
        <Layer
          onClose={this._onCloseLayer.bind(this)}
          closer={true}
          align="top"
        >
          <Form>
  ...
  render() {
    const { credential, errorMsg, error } = this.state;
    ...
    return (
      <Box
        ...
        wrap={true}
        full="vertical"
        texture="url(/andon-system/static/img/cover.jpg)"
      >
        <Box
          align="end"
          justify="end"
          pad={{ horizontal: "large", vertical: "large", between: "large" }}
        >
          <Box
            size="auto"
            align="center"
          ...
          >
            <Heading>
              {this.localeData.APP_NAME_FULL}
            </Heading>
            {logging}
            <Form>
              <FormFields>
                <FormField
                  label={this.localeData.login_email}
                  error={error.email}
                >
                  <input
                    type="text"
                ...
                <FormField
                  label={this.localeData.login_password}
